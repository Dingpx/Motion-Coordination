...
def init_data(opt,acts):
	...
	# with open('test_long.pkl', 'wb') as f:
...
def test(test_loader,netG,act,is_cuda=False,output_n=10,input_n=10):
	...
	for i, (inputs,outputs,raw_outputs,raw_inputs) in enumerate(test_loader):
		...
		np.save('/home/dpx/Abalation/vis/h36m/long_term/input/input_'+act,raw_inputs1.detach().cpu().numpy())
		raw_outputs1 = raw_outputs.contiguous().view(n,outputs_length,-1,3)
		np.save('/home/dpx/Abalation/vis/h36m/long_term/gt/gt_'+act,raw_outputs1.detach().cpu().numpy())
		...
		np.save('/home/dpx/Abalation/vis/h36m/long_term/my_pred/pred_'+act,pred_3d.detach().cpu().numpy())
